...
| **Wikidata Query Service** (varredura por país e categoria, e `wikibase:around`) — desde #219, modelada como âmbito na #220 e **ainda sem consumidor** | Itens de uma classe num país (`P31/P279*` + `P17`) com coordenada, artigo em inglês e `P131`; e o município mais próximo (`Q15284`) até 30 km para o item sem `P131` | **Uma consulta por classe**, não por categoria: 41 classes num só `VALUES` deram 504 em PT, BR e IT; oito levaram 16,4 s em PT e falharam em IT; uma classe responde em segundos. Mais uma consulta `around` por item sem `P131`. ≥600 ms entre chamadas — o WDQS devolve 502 depois de ~5 seguidas — e timeout de 60 s | Classe que não responde entra em `classesFailed` e a varredura segue; item sem resposta fica **sem cidade e contado**, nunca com uma inventada; três falhas seguidas param a perna geográfica. Corpo truncado (200 com JSON cortado, que a cache do WDQS chega a servir) conta como falha retentável |
